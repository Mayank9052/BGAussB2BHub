...
const SCOOTY_API = "/api/ScootyInventory";
const PAGE_SIZE = 10;
...
export default function RoadPrice() {
  ...
  const [allCitiesModal, setAllCitiesModal] = useState<{
    ...
    rows: AllCityRow[];
    loading: boolean;
  }>({ open: false, scootyId: null, scootyLabel: "", rows: [], loading: false });
  ...
  try {
    ...
    // 1. Get scooties
    const scootyRes = await axios.get<Scooty[]>(`${SCOOTY_API}/scooties`);
    const scootiesList = scootyRes.data;
    setScooties(scootiesList);

    if (scootiesList.length === 0) {
      setData([]);
      return;
    }

    // 2. Get all-cities for each scooty
    const allCityResults = await Promise.allSettled(
      scootiesList.map((s) =>
        axios.get<AllCityRow[]>(`${API}/${s.scootyId}/all-cities`)
      )
    );

    // 3. Build base rows
    const baseRows: RoadPriceItem[] = [];

    allCityResults.forEach((res, idx) => {
      if (res.status === "fulfilled") {
        const scooty = scootiesList[idx];

        res.value.data.forEach((row) => {
          baseRows.push({
            id: -1, // temp
            scootyId: scooty.scootyId,
            modelName: scooty.modelName,
            variantName: scooty.variantName,
            colourName: scooty.colourName,
            cityId: row.cityId,
    ...
    // 4. Fetch details ONLY for visible (first page)
    const firstPage = baseRows.slice(0, PAGE_SIZE);

    const detailResults = await Promise.allSettled(
      firstPage.map((r) =>
        axios.get<RoadPriceItem>(`${API}/${r.scootyId}/city/${r.cityId}`)
      )
    );

    detailResults.forEach((res, i) => {
      if (res.status === "fulfilled") {
        baseRows[i] = res.value.data;
      }
    });
    ...
    setAlert({ type: "error", msg: "Failed to load road price data." });
  } finally {
    setLoading(false);
  }
}, []);
 ...
 useEffect(() => {
  const loadDetailsForPage = async () => {
    ...
    results.forEach((res, i) => {
      if (res.status === "fulfilled") {
        updated[start + i] = res.value.data;
      }
    });
  ...
  if (data.length) loadDetailsForPage();
}, [currentPage]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAlert({
        type: "success",
        msg: editingId
          ? "Road price updated successfully."
          : "Road price saved successfully.",
      });
      resetForm();
      fetchData();
    } catch (err: any) {
  ...
  /* ── EDIT — fetch detail then populate form ── */
  const handleEdit = async (row: RoadPriceItem) => {
  try {
    let d = row;

    if (!row.exShowroomPrice) {
      const res = await axios.get<RoadPriceItem>(
        `${API}/${row.scootyId}/city/${row.cityId}`
      );
      d = res.data;
    }
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fd.append("file", f);
    try {
      const res = await axios.post<string>(`${API}/import`, fd, {
  ...
  /* ── STATS ── */
  const uniqueScooties = new Set(data.map((d) => d.scootyId)).size;
  const uniqueCities = new Set(data.map((d) => d.cityId)).size;
    ...
    <div className="scr-page">
      ...
      <header className="pro-navbar">
        <div className="pro-left">
          ...
          <div className="pro-text">
            ...
            <span className="pro-page">Road Price</span>
          </div>
        </div>
      ...
      <div className="scr-container">
        ...
        {alert && (
          <div
            className={`scr-alert ${
              alert.type === "success" ? "scr-alert-success" : "scr-alert-error"
            }`}
          >
            <span>{alert.type === "success" ? "✓" : "✕"}</span>
            {alert.msg}
            <button className="scr-alert-close" onClick={() => setAlert(null)}>
              ×
            </button>
          </div>
        )}
        ...
        <div className="scr-banner">
          <div className="scr-banner-icon">🛵</div>
          ...
          <div className="scr-chips">
            <div className="scr-chip scr-chip-total">
              <strong>{data.length}</strong> Total Records
            </div>
            <div className="scr-chip scr-chip-stock">
              <strong>{uniqueScooties}</strong> Scooties
            </div>
            <div className="scr-chip scr-chip-out">
        ...
        <div className="scr-top-grid">
          ...
          <div className="scr-left-col">
            <div className="scr-form-card">
              ...
              <div className="scr-form-header">
                ...
                <p>
                  {editingId
                    ? "Update the selected road price entry below"
                    : "Fill in details to set the on-road price for a scooty in a city"}
                </p>
              </div>
              ...
              <div className="scr-form-body">
                        ...
                        {cities.map((c) => (
                          <div
                            ...
                            onMouseDown={(e) => {
                              ...
                              setCitySearch(`${c.cityName}, ${c.stateName}`);
                              setCityDropdownOpen(false);
                            }}
                          >
                ...
                {/* Row 2: Ex-Showroom | RTO | Insurance | Other */}
                ...
                {/* Row 3: Valid From + On-Road Preview + Buttons */}
                <div className="scr-row-bottom">
                  <div style={{ display: "flex", gap: "8px", alignItems: "flex-end", flex: 1 }}>
                    ...
                    {computedOnRoad > 0 && (
                      <div className="rp-onroad-preview">
                        ...
                        <span className="rp-onroad-value">
                          ₹ {computedOnRoad.toLocaleString("en-IN")}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="scr-form-btns">
                    {editingId && (
                      <button className="scr-btn-cancel" onClick={resetForm}>
                        Cancel
                      </button>
                    )}
          ...
          <div className="scr-bulk-card">
            <div className="scr-bulk-header">
              <div className="scr-bulk-icon">📊</div>
              <h2>Bulk Operations</h2>
              <p>Download template or import road prices via Excel</p>
            </div>
            ...
            <div className="scr-bulk-body">
              <div className="scr-bulk-section">
                <div className="scr-bulk-section-info">
                  ...
                  <span className="scr-bulk-desc">
                    Get the Excel template with columns: ScootyId, CityId,
                    ExShowroomPrice, RtoCharges, InsuranceAmount, OtherCharges,
                    ValidFrom.
                  </span>
                </div>
              ...
              <div className="scr-bulk-section">
                <div className="scr-bulk-section-info">
                  ...
                  <span className="scr-bulk-desc">
                    Upload a filled Excel file to bulk insert or update road
                    prices. Existing records (same ScootyId + CityId) will be
                    updated automatically.
                  </span>
                </div>
                <button
                  className="scr-btn-imp"
        ...
        <div className="scr-table-card">
          <div className="scr-table-header">
            ...
            <div className="scr-header-pagination">
              <span className="scr-pg-info-inline">
              ...
              <div className="scr-pg-btns">
                ...
                {buildPages(safeCurrentPage, totalPages).map((p, i) =>
                  p === "…" ? (
                    <span key={`e${i}`} className="scr-pg-ellipsis">
                      …
                    </span>
                  ) : (
                    <button
                      key={p}
                      className={`scr-pg${safeCurrentPage === p ? " active" : ""}`}
            ...
            <div className="scr-table-controls">
              <div className="scr-search-wrap">
                ...
                <input
                  ...
                  placeholder="Search model, city, state…"
                  value={tableSearch}
                  onChange={(e) => {
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8}>
                      <div className="scr-empty">
                        ...
                        <span className="scr-empty-sub">
                          {tableSearch
                            ? "No records match your search. Try different keywords."
                            : "Add your first road price record using the form above."}
                        </span>
                      </div>
                    </td>
                  </tr>
                  ...
                  paginated.map((d, idx) => (
                    <tr key={`${d.scootyId}-${d.cityId}`}>
                      ...
                      <td data-label="Scooty">
                        <div className="scr-model-cell">
                          <div className="scr-model-avatar">
                            {d.modelName?.charAt(0).toUpperCase() ?? "?"}
                          </div>
                      ...
                      <td data-label="On-Road Price">
                        <span className="scr-price rp-onroad-pill">
                      ...
                      <td data-label="Actions">
                        <div className="scr-row-acts">
                          <button
                            className="scr-act-btn scr-act-cities"
                            onClick={() => openAllCities(d)}
                            title="View all city prices for this scooty"
                          >
                            <svg
                              viewBox="0 0 24 24"
                              fill="none"
        ...
        </div>{/* end scr-table-card */}

      </div>{/* end scr-container */}
          ...
          <div className="rp-modal" onClick={(e) => e.stopPropagation()}>
            <div className="rp-modal-header">
              ...
              <button
                ...
                onClick={() => setAllCitiesModal((p) => ({ ...p, open: false }))}
              >
                ×
              </button>
            </div>
                      ...
                      <tr key={r.cityId}>
                        ...
                        <td>
                          <span className="scr-price">
                            ₹ {Number(r.onRoadPrice).toLocaleString("en-IN")}
                          </span>
                        </td>
                      </tr>
                    ))}
